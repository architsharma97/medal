...
from agents import SACAgent, MEDALBackwardAgent
from backend.timestep import ExtendedTimeStep

torch.backends.cudnn.benchmark = True

def make_forward_agent(obs_spec, action_spec, cfg):
...
def make_backward_agent(obs_spec, action_spec, cfg):
    cfg.obs_shape = obs_spec.shape
    cfg.action_shape = action_spec.shape

    return MEDALBackwardAgent(
...
class Workspace:
    def __init__(self, cfg):
        ...
        self.forward_agent = make_forward_agent(self.train_env.observation_spec(),
                                                self.train_env.action_spec(),
                                                self.cfg.forward_agent)
        self.backward_agent = make_backward_agent(self.train_env.observation_spec(),
                                                  self.train_env.action_spec(),
    ...
    def setup(self):
        ...
        if self.cfg.simple_buffer:
            self.replay_storage_f = SimpleReplayBuffer(data_specs=data_specs,
            ...
            self.backward_pos = ReplayBufferStorage(data_specs,
                                                    self.work_dir / 'backward_buffer_pos')

            self.backward_neg = ReplayBufferStorage(data_specs,
                                                    self.work_dir / 'backward_buffer_neg')
            ...
            self.pos_loader = make_replay_loader(
                self.work_dir / 'backward_buffer_pos', self.cfg.discriminator.positive_buffer_size,
                self.cfg.discriminator.batch_size // 2, self.cfg.replay_buffer_num_workers,
                self.cfg.save_snapshot)
            self.neg_loader = make_replay_loader(
                self.work_dir / 'backward_buffer_neg', self.cfg.discriminator.negative_buffer_size,
                self.cfg.discriminator.batch_size // 2, self.cfg.replay_buffer_num_workers,
                self.cfg.save_snapshot)

        self._forward_iter, self._backward_iter, self._pos_iter, self._neg_iter = None, None, None, None
    ...
    def train(self):
        ...
        self.replay_storage_b.add_offline_data(self.forward_demos, init_action)
        for goal in self.goal_states:
            self.replay_storage_b.add_offline_data(self.get_relabeled_demos(self.backward_demos, goal), init_action)
        ...
        while train_until_step(self.global_step):
            ...
            if not seed_until_step(self.global_step):
                ...
                if self._global_step % self.cfg.discriminator.train_interval == 0:
                    for k in range(self.cfg.discriminator.train_steps_per_iteration):
                        metrics = self.backward_agent.update_discriminator(self.pos_iter, self.neg_iter)
                    self.logger.log_metrics(metrics, self.global_frame, ty='train')
            ...
            if (episode_step + 1) % self.cfg.policy_switch_frequency == 0 or (cur_policy == 'forward' and self.train_env.is_successful(updated_obs)):
                switch_policy = True
                updated_step_type = 2
